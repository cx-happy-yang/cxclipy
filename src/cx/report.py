...
def generate_sast_report(
    ...
    sast_results_collection = get_sast_results_by_scan_id(
        scan_id=scan_id, offset=offset, limit=limit,sort=["+status", "+severity","-queryname"], state=["TO_VERIFY"], include_nodes=False,
    )
        ...
        report_content.append(
            {
                ...
                "SrcFileName": result.nodes[0].file_name,
                "Line": result.nodes[0].line,
                "Column": result.nodes[0].column,
                "NodeId": result.nodes[0].node_hash,
                "Name": result.nodes[0].full_name,
                "DestFileName": result.nodes[-1].file_name,
                "DestLine": result.nodes[-1].line,
                "DestColumn": result.nodes[-1].column,
                "DestNodeId": result.nodes[-1].node_hash,
                "DestName": result.nodes[-1].full_name,
